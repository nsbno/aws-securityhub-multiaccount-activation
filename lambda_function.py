...
def lambda_handler(event, context):
    ...
    enabled_regions = os.environ["enabled_regions"]
    enable_standards = os.environ["enable_standards"]
    bucket = 'securityhub-members'
    key = 'enable.csv'
    ...
    for line in lines:
      ...
      aws_account_dict[line[0]] = line[1]
    ...
    master_session = assume_role(master_account, role_name)
            ...
            while len(standards_to_verify) > 0:
                if (int(time.time()) - start_time) > 100:
                    print("Timeout waiting for READY state enabling standards {standards} in region {region} for account {account}, last state: {status}"
                          .format(standards=compliance_standards_arns, region=aws_region, account=master_account, status=standards_status))
                    break
                ...
                for enabled_standard in enabled_standards['StandardsSubscriptions']:
                    ...
                    if enabled_standard_arn in standards_to_verify and enabled_standard_status == 'READY':
                        print("Finished enabling stanard {} on account {} for region {}".format(enabled_standard_arn, master_account, aws_region))
    ...
    for account in aws_account_dict.keys():
        ...
        try:
            ...
            session = assume_role(account, role_name)
